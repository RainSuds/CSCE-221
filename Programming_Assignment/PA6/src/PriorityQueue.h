...
private:
    ...
    void upheap(size_type index) {
        if (index != 0) {
            size_type my_parent = parent(index);

            if (this->comp(this->c.at(my_parent), this->c.at(index))) {
                value_type temp = std::move(this->c.at(my_parent));
                this->c.at(my_parent) = std::move(this->c.at(index));
                this->c.at(index) = std::move(temp);

                this->upheap(my_parent);
            }
        }
    ...
    void downheap(size_type index) {
        // TODO
        size_type my_left = left_child(index);

        if (my_left < this->c.size()) { // left exist
            size_type largest = index;

            if (this->comp(this->c.at(index), this->c.at(my_left))) { // left > index
                largest = my_left;
            }

            size_type my_right = right_child(index);
            if (my_right < this->c.size() && this->comp(this->c.at(largest), this->c.at(my_right))) {
                largest = my_right;
            }

            if (largest != index) {
                value_type temp = std::move(this->c.at(largest));
                this->c.at(largest) = std::move(this->c.at(index));
                this->c.at(index) = std::move(temp);
...
public:
     ...
     */
    size_type size() const { /* TODO */ return this->c.size(); }
    ...
    void pop() {
        // TODO
        this->c.front() = std::move(this->c.back());
        this->c.pop_back();
        this->downheap(0);
    }
};